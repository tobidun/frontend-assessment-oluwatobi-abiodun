...
export const ProductList = () => {
  ...
  const {
    ...
    queryKey: ['products', searchQuery, activeFilter],
  ...
  if (isLoading && !data) {
    return (
      <div className="pt-2">
        <ItemListSkeleton type="products" count={8} mode={viewMode} />
      </div>
    )
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center px-4 animate-in fade-in duration-700">
         ...
         <p className="text-slate-500 max-w-sm mx-auto font-medium text-sm leading-relaxed">
           The current registry contains no products matching your search parameters.
         </p>
      </div>
    )
  }
      ...
      <div className={viewMode === 'grid' 
        ...
        {data?.pages.map((page, pageIdx) => (
          <Fragment key={pageIdx}>
            {page.products?.map((item: DummyProduct, idxLog: number) => (
              <ProductCard
                key={item.id}
                item={item}
      ...
      <div ref={ref} className="pt-16 pb-32 min-h-[200px]">
        {hasNextPage && (
          <div className="space-y-4">
             <ItemListSkeleton type="products" count={viewMode === 'grid' ? 4 : 2} mode={viewMode} />
          </div>
        )}
