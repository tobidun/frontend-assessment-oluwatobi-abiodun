...
export const PostList = () => {
  ...
  const { searchQuery, activeFilter, viewMode } = useExplorerStore()
  ...
  const {
    ...
    queryKey: ['posts', searchQuery, activeFilter],
    queryFn: ({ pageParam = 0 }) => fetchPosts({ 
      pageParam, 
      limit: 12, 
      query: searchQuery,
  ...
  if (isLoading && !data) {
    return (
      <div className="pt-2">
        <ItemListSkeleton type="posts" count={8} mode={viewMode} />
      </div>
    )
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center px-4 animate-in fade-in duration-700">
         ...
         <p className="text-slate-500 max-w-sm mx-auto font-medium text-sm leading-relaxed">
           The current registry contains no reports matching your search parameters.
         </p>
      </div>
    )
  }
      ...
      <div className={viewMode === 'grid' 
        ...
        {data?.pages.map((page, pageIdx) => (
          <Fragment key={pageIdx}>
            {page.posts?.map((item: DummyPost, idxLog: number) => (
              <PostCard
                key={item.id}
                item={item}
      ...
      <div ref={ref} className="pt-16 pb-32 min-h-[200px]">
        {hasNextPage && (
          <div className="space-y-4">
             <ItemListSkeleton type="posts" count={viewMode === 'grid' ? 4 : 2} mode={viewMode} />
          </div>
        )}
