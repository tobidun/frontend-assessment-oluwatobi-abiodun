...
export default function PostDetailClient({ id }: PostDetailClientProps) {
  ...
  const auditLogs = [
    { id: 1, action: 'Report Validated', date: '1 hour ago', icon: CheckCircle2, color: 'text-green-600' },
    { id: 2, action: 'Author Identity Verified', date: '4 hours ago', icon: Clock, color: 'text-blue-600' },
    { id: 3, action: 'Content Integrity Check', date: '6 hours ago', icon: CheckCircle2, color: 'text-green-600' },
    { id: 4, action: 'Registry Sync Success', date: 'Yesterday', icon: ExternalLink, color: 'text-primary-600' },
  ]
  ...
  if (!item) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center px-4">
        <h1 className="text-2xl font-bold text-slate-900">Report Entry Not Found</h1>
        <Link href="/posts" className="text-primary-600 font-bold hover:underline">Return to Registry</Link>
      </div>
    </div>
  )
            ...
            <div className="space-y-6 mb-12">
              <div className="flex flex-col gap-2">
                 <div className="flex items-center gap-3">
                   <div className="px-2.5 py-1 rounded-md bg-amber-600 text-white text-[10px] font-black uppercase">REPORT</div>
              ...
              <p className="text-xl text-slate-500 font-medium leading-relaxed">{item.body}</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10 border-t border-slate-100 pt-10">
              <div className="flex flex-col gap-2">
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Author ID</span>
                <div className="text-base font-bold text-slate-900">{item.userId || 'Unknown'}</div>
              </div>
              <div className="flex flex-col gap-2">
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Reactions</span>
                <div className="text-base font-bold text-slate-900">{item.reactions?.likes || 0} Likes, {item.reactions?.dislikes || 0} Dislikes</div>
              </div>
              <div className="flex flex-col gap-2">
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Tags</span>
                <div className="text-base font-bold text-slate-900">{item.tags?.join(', ') || 'N/A'}</div>
              </div>
              <div className="flex flex-col gap-2">
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Views</span>
                <div className="text-base font-bold text-slate-900">{item.views || 0}</div>
              </div>
            </div>
            ...
            {showAudit && (
              <div className="mt-8 space-y-4 animate-in slide-in-from-top-4 duration-500">
                <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
                  <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest mb-6">Access Logs</h3>
                    ...
                    {auditLogs.map((log) => (
                      <div key={log.id} className="flex items-center gap-4">
                         ...
                         <div>
                            ...
                            <div className="text-[10px] font-bold text-slate-400 uppercase">{log.date}</div>
                         </div>
                      </div>
                    ))}
                  </div>
